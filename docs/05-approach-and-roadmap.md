...
| **M11 — test code under the rules, type-only exceptions** (done) | A `source_roots` entry outside `package` contributes its top-level packages as components (`grimp.build_graph(package, *extras)`, the multi-package call M9 proved for siblings), so a rule can reach test code; `[[archview.exceptions]]` gains `kind = "type_only"`, exempting only a type-checking-only import between a pair (ADR 0015) | `["src", "."]` makes `tests/test_live.py` a component a rule can name, expressing "no test may import a plugin, except this one" with no ruff `per-file-ignores`; a value import between an exempted pair still fails when `kind = "type_only"`; `tests/test_self_check.py` still asserts zero warnings (done 2026-09-22) |
